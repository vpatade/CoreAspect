...
class AdviceWizardFirstPage extends WizardPage implements Listener {
	...
	AdviceWizardFirstPage() {
		super("Activity Page1");
	...
	public void createControl(Composite parent) {
		...
		implementationLabel = new Label(composite, SWT.PUSH);
		...
		GridData gd3 = new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING, GridData.VERTICAL_ALIGN_FILL, false, false, 1,
				1);
		gd3.widthHint = 100;
		...
		GridData gd51 = new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING, GridData.VERTICAL_ALIGN_FILL, false, false,
				1, 1);
		gd51.widthHint = 250;
	...
	public void handleEvent(Event e) {
		if (e.widget.equals(browseButton)) {
			...
			IWizardDescriptor descriptor = PlatformUI.getWorkbench().getNewWizardRegistry()
					.findWizard("org.eclipse.jdt.ui.wizards.NewClassCreationWizard");
			try {
				if (descriptor != null) {
		...
		if (e.widget.equals(pointcutNameCombo) || e.widget.equals(whereCombo) || e.widget.equals(classNameText)
				|| e.widget.equals(adviceNameText)) {
	...
	boolean isDone() {
		if (adviceNameText.getText().compareTo("") != 0 && !DataModel.hasAdvice(adviceNameText.getText())) {
			if (classNameText.getText().compareTo("") != 0 && whereCombo.getText().compareTo("") != 0) {
				return true;
			}
		}
		return false;
