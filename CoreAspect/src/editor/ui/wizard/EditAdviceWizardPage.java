...
class EditAdviceWizardPage extends WizardPage implements Listener{
  
  Label adviceNameLabel;
  Text adviceNameText;
  Label pointcutNameLabel;
  Combo pointcutNameCombo;
  Text queryText;
  Label whereLabel;
  Combo whereCombo;
  Label docuLabel;
  ...
  Composite c41;
  GridData gd411; 
  EditAdviceWizardPage() {
    super("Edit Advice Page");
    setTitle("Advice Editing");
    setDescription("Edit the Existing Advice");
  }
  ...
  public void createControl(Composite parent) {
    ...
    pointcutNameCombo=new Combo(cc1,SWT.NONE|SWT.READ_ONLY);
    pointcutNameCombo.setLayoutData(gd111);
    for(int i=0;i<DataModel.pointCutNames.size();i++)
    	pointcutNameCombo.add(DataModel.pointCutNames.get(i));
	...
	c41=new Composite(c4444,SWT.NULL);
	 ...
	 pointcutNameCombo.addListener(SWT.Modify, this);
  ...
  public void addToPage(){
	  String ad=EditAdviceWizardDriver.ad;
	  oldAdviceName=ad;
	  ind=DataModel.getADIndex(ad);
	  pointcutNameCombo.setText(DataModel.advicePointCutNames.get(ind));
	  adviceNameText.setText(DataModel.adviceNames.get(ind));
	  queryText.setText(DataModel.getQuery(pointcutNameCombo.getText()));
	  whereCombo.setText(DataModel.adviceWhen.get(ind));
	  docuText.setText(DataModel.adviceDocumentations.get(ind));
	  classNameText.setText(DataModel.adviceImplementations.get(ind));
  }
...
@Override
  public void handleEvent(Event e) {
	  if(e.widget.equals(pointcutNameCombo)){
		  	queryText.setText(DataModel.getQuery(pointcutNameCombo.getText()));
	...
	if(e.widget.equals(pointcutNameCombo)||e.widget.equals(whereCombo)
		||e.widget.equals(classNameText)||e.widget.equals(adviceNameText)){
		if(e.widget.equals(adviceNameText)){
			if(DataModel.hasAdvice(adviceNameText.getText())&&adviceNameText.getText().compareToIgnoreCase(oldAdviceName)!=0){
